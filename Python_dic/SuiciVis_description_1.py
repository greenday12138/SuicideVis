...
#结果路径
file_path=r'C:\Users\lenovo\Data_visualization\SuiciVis\data\result.json'
...
结果变量：
    ...
    #键：标签名，为string对象，按国家/年份/性别/数据项名顺序。例，在dict对象country_year_sex_mean,country_year_sex_mode中一键为'Uzbekistan2014malemode'
    #值：标签对应的数据项值

变量名：
    1.均为小写
    2.名称即表明键值。如某一键值为'Albania1987femalemean'的平均值dict对象，其名称为country_year_sex_mean
...
def MeanAge( mean_age, no,mean):
    '''
    :param mean_age:一般对应['mean_age']列，可视为np.average()第一个参数。如df['mean_age],df1['mean_age].
    :param no: 一般对应['suicides_no']列，可视为np.average()weights参数。如df['suicides_no'],df1['suicides_no']
    :param mean: dict对象，键为标签，值为对应需要计算的平均值。
    :return: None
    '''
    for i in range(len(mean_age)):
        if np.sum(list(no[i][1])):
            ...
            mean[ToString(mean_age[i][0])+'mean']=np.average(list(mean_age[i][1]),axis=0,weights=list(no[i][1]))
        else:
            mean[ToString(mean_age[i][0])+'mean']=None
    return
...
def CountryYearSexMode(r,mode):
    for i in range(len( r['max'])):
        ...
        mode[ToString(r['max'].index[i])+'mode']=df['mean_age'][temp5[0]]
    return
...
def CountryYearMode(r,mode):
    for i in range(len(r['max'])):
        ...
        temp5=returnMatches(temp3,temp4)
        #mode[tuple(r['max'].index[i].split('\n')+['mode'])]=df['mean_age'][temp5[0]]
        mode[r['max'].index[i]+'mode']=df['mean_age'][temp5[0]]
    return
...
def CountryMode(r,mode):
    for i in range(len(r['max'])):
        index3,index4=np.arange(len(tuple(df['suicides/100k pop']))),np.arange(len(tuple(df['country'])))
        temp3,temp4=index3[tuple(df['suicides/100k pop'])==r['max'][i]],returnMatchesIndex(tuple(df['country']),r['max'].index[i])
        temp5=returnMatches(temp3,temp4)
        #mode[tuple(r['max'].index[i].split('\n')+['mode'])]=df['mean_age'][temp5[0]]
        mode[r['max'].index[i]+'mode']=df['mean_age'][temp5[0]]
    return
...
fw = open(file_path, "w")  # 打开json文件

raw=dict()
raw.update(country_year_sex_mean)
raw.update(country_year_mean)
raw.update(country_sex_mean)
raw.update(country_mean)
raw.update(country_year_sex_mode)
raw.update(country_year_mode)
raw.update(country_sex_mode)
raw.update(country_mode)
